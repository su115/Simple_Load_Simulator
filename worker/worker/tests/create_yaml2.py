...
def _get_dict_randomly(): # headers,params,data with random values
    rand_data = ["red","yellow","blue","white","black","orange","pink","green","grey","brown"]
...
def _add_req(instance,url, name=None,method=None,headers=None,params=None,data=None ):
...
def get_yaml_data_related(table): # related to test_server_url_table
    #l_sessions = []
        ...
        for location in locations:
            url =item['protocol']+'://'+ test_server_ip + item['port'] + location['location']
            print("+"*23)
            print("LOCATION",location)
            _add_req(instance=session,
                     url=url,
